...
export default function App() {
  ...
  // 사용자 정보 (승현 / 상오니 커플 연동)
  const [currentUser] = useState({ name: '승현', email: 'seunghyun@couple.com', coupleId: 'couple-1012' });
  const [partnerUser] = useState({ name: '상오니', email: 'sangoni@couple.com' });
  ...
  useEffect(() => {
    async function fetchDailyData() {
      try {
        const dietRef = doc(db, "CoupleDiets", `${currentUser.coupleId}_${selectedDate}_${currentUser.email}`);
        ...
        const workoutQuery = query(collection(db, "CoupleWorkouts"), where("coupleId", "==", currentUser.coupleId), where("ownerEmail", "==", currentUser.email), where("targetDate", "==", selectedDate));
        ...
        const fineQuery = query(collection(db, "CouplePenalties"), where("coupleId", "==", currentUser.coupleId));
    ...
    async function fetchMonthData() {
      try {
        ...
        for (let d = new Date(firstDay); d <= lastDay; d.setDate(d.getDate() + 1)) {
          ...
          const docRef = doc(db, "CoupleDiets", `${currentUser.coupleId}_${dateString}_${currentUser.email}`);
  ...
  const handleDietChange = async (newStatus) => {
    ...
    try {
      const docId = `${currentUser.coupleId}_${selectedDate}_${currentUser.email}`;
      ...
      if (newStatus === '실패') {
        const penaltyKey = `${currentUser.email}_${selectedDate}_Diet`;
  ...
  return (
    <div style={{ maxWidth: '480px', margin: '0 auto', background: '#f4fcfc', minHeight: '100vh', paddingBottom: '80px', fontFamily: '-apple-system, BlinkMacSystemFont, sans-serif', color: '#333' }}>
      ...
      <div style={{ background: 'linear-gradient(135deg, #008080 0%, #20b2aa 100%)', color: '#fff', padding: '18px', textAlign: 'center', borderBottomLeftRadius: '24px', borderBottomRightRadius: '24px', boxShadow: '0 4px 12px rgba(0,0,0,0.1)' }}>
        ...
        <p style={{ fontSize: '12px', margin: 0, opacity: 0.9 }}>승현 & 상오니의 완벽한 휴가를 위한 커플 미션</p>
      </div>
      ...
      <div style={{ padding: '16px' }}>
        ...
        {currentTab === 'home' && (
          <div>
            ...
            <div style={{ background: '#fff', padding: '16px', borderRadius: '16px', marginBottom: '16px', boxShadow: '0 2px 8px rgba(0,0,0,0.05)' }}>
              <div style={{ fontWeight: 'bold', fontSize: '14px', color: '#ff7f50', marginBottom: '10px' }}>💖 상대방 ({partnerUser.name}) 응원하기</div>
        ...
        {currentTab === 'settings' && (
          <div style={{ background: '#fff', padding: '16px', borderRadius: '16px', boxShadow: '0 2px 8px rgba(0,0,0,0.05)' }}>
            <h3 style={{ fontSize: '15px', color: '#008080', marginTop: 0 }}>⚙️ 앱 설정</h3>
            <div style={{ fontSize: '12px', display: 'flex', flexDirection: 'column', gap: '8px', color: '#555' }}>
              <div><b>현재 연결된 커플 ID:</b> {currentUser.coupleId}</div>
              <div><b>내 이메일:</b> {currentUser.email}</div>
              <div><b>상대방 이메일:</b> {partnerUser.email}</div>
              <div><b>푸켓 여행일:</b> {travelDate}</div>
              <div><b>타임존:</b> Asia/Seoul (KST)</div>
            </div>
          </div>
        )}
        ...
        <div style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', background: 'rgba(0,0,0,0.6)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 1000, padding: '16px' }}>
          <div style={{ background: '#fff', padding: '24px', borderRadius: '20px', textAlign: 'center', maxWidth: '320px', border: '4px solid #ff7f50' }}>
            ...
            <button 
              ...
              style={{ background: '#008080', color: '#fff', border: 'none', padding: '8px 16px', borderRadius: '10px', fontWeight: 'bold', cursor: 'pointer' }}
            >
              확인 🥳
            </button>
          </div>
